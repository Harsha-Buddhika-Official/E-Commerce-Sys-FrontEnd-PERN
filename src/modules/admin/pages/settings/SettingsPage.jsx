...
// ─── Result Overlay ───────────────────────────────────────────────────────────
function ResultOverlay({ type, message, onClose }) {
    ...
    return (
        ...
        >
            <div
                className="bg-white rounded-2xl w-full relative flex flex-col items-center text-center"
                style={{
                    maxWidth:  400,
            ...
            >
                ...
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 flex items-center justify-center w-8 h-8 rounded-xl border cursor-pointer transition-all hover:bg-gray-50"
                    style={{ borderColor: "#ebebeb", backgroundColor: "transparent", color: "#aaa" }}
                >
                ...
                <div
                    ...
                    style={{ backgroundColor: isSuccess ? "#f0fdf4" : "#fef2f2" }}
                >
                    {isSuccess
                        ? <CheckCircleOutlinedIcon style={{ fontSize: 36, color: "#16a34a" }} />
                        : <ErrorOutlineOutlinedIcon style={{ fontSize: 36, color: "#e53935" }} />
                    }
                </div>

                {/* Title */}
                <p style={{ ...SORA, fontSize: 17, fontWeight: 900, color: "#111", margin: "0 0 8px", letterSpacing: "-0.3px" }}>
                    {isSuccess ? "Password Updated!" : "Update Failed"}
                </p>

                {/* Message */}
                <p style={{ ...INTER, fontSize: 13, color: "#666", lineHeight: 1.7, margin: "0 0 24px" }}>
                    {message}
                </p>
                ...
                >
                    {isSuccess
                        ? <><CheckCircleOutlinedIcon style={{ fontSize: 16 }} /> Got it</>
                        : <><CloseOutlinedIcon style={{ fontSize: 16 }} /> Try Again</>
                    }
                </button>
            </div>
        </div>
...
function PasswordField({ label, value, onChange, placeholder, show, onToggle, error, required }) {
...
export default function SettingsPage() {
    ...
    // overlay state
    const [overlay, setOverlay] = useState(null); // { type: 'success' | 'error', message: string }

    const { saving, saved, error: apiError, updatePassword, reset } = useUpdatePassword();
    ...
    const validate = () => {
        ...
        else if (newPw === currentPw)   e.newPw = "New password must differ from current password.";
    ...
    const handleSave = async () => {
        ...
        if (success) {
            ...
            setOverlay({
                type:    "success",
                message: "Your password has been updated successfully. Use your new password the next time you log in.",
            });
        } else {
            setOverlay({
                type:    "error",
                message: apiError || "Something went wrong while updating your password. Please check your current password and try again.",
            });
        }
    };
    ...
    return (
        <div className="h-full overflow-y-auto bg-[#f5f5f5] p-5 lg:p-6">
            ...
            <div className="flex items-center justify-between mb-6">
                <div>
                    <p style={{ ...INTER, fontSize: 11, color: "#aaa", fontWeight: 500 }}>System / Settings</p>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-5">
                ...
                <div className="xl:col-span-1">
                    <SectionCard>
                        <p style={{ ...INTER, fontSize: 10, fontWeight: 700, color: "#bbb", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 12 }}>
                            Settings
                        </p>
                        <div className="flex flex-col gap-1">
                            <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl" style={{ backgroundColor: "#111" }}>
                                <SecurityOutlinedIcon style={{ fontSize: 16, color: "#fff" }} />
                                <span style={{ ...INTER, fontSize: 13, fontWeight: 700, color: "#fff" }}>Password</span>
                            </div>
                            {[
                                { icon: <SettingsOutlinedIcon style={{ fontSize: 16 }} />, label: "General",     soon: true },
                                { icon: <ShieldOutlinedIcon   style={{ fontSize: 16 }} />, label: "Permissions", soon: true },
                            ].map(({ icon, label, soon }) => (
                                <div key={label} className="flex items-center justify-between gap-3 px-4 py-2.5 rounded-xl" style={{ opacity: 0.4 }}>
                                    <div className="flex items-center gap-3">
                                        <span style={{ color: "#888" }}>{icon}</span>
                                        <span style={{ ...INTER, fontSize: 13, fontWeight: 600, color: "#555" }}>{label}</span>
                                    </div>
                                    {soon && (
                                        <span className="px-2 py-0.5 rounded-full" style={{ ...INTER, fontSize: 9, fontWeight: 700, backgroundColor: "#f0f0f0", color: "#aaa" }}>
                                            Soon
                                        </span>
                                    )}
                                </div>
                        ...
                        <div className="flex flex-col gap-5">
                            ...
                            {newPw && (
                                <div className="-mt-2">
                                    <div className="flex items-center justify-between mb-2">
                                        ...
                                        <span style={{ ...INTER, fontSize: 11, fontWeight: 700, color: strength.color }}>{strength.label}</span>
                                    </div>
                                    <div className="flex gap-1 mb-3">
                                        {[1, 2, 3, 4].map((i) => (
                                            <div
                                                ...
                                                style={{ height: 5, backgroundColor: i <= strength.level ? strength.color : "#f0f0f0" }}
                                            />
                                        ))}
                                    </div>
                                        ...
                                        {RULES.map(({ test, label }) => (
                                            <div key={label} className="flex items-center gap-2">
                                                <div
                                                    ...
                                                    style={{ backgroundColor: test(newPw) ? "#16a34a" : "#e0e0e0" }}
                                                />
                                                <span style={{ ...INTER, fontSize: 11, color: test(newPw) ? "#16a34a" : "#bbb", fontWeight: 500 }}>
                                                    {label}
                                                </span>
                                            </div>
                                        ))}
                            ...
                            {confirmPw && newPw && (
                                <div className="flex items-center gap-2 -mt-2">
                                    <div
                                        ...
                                        style={{ backgroundColor: newPw === confirmPw ? "#16a34a" : "#e53935" }}
                                    />
                                    <span style={{ ...INTER, fontSize: 11, fontWeight: 600, color: newPw === confirmPw ? "#16a34a" : "#e53935" }}>
                            ...
                            <div className="flex justify-end pt-2">
                                <button
                                    ...
                                    onMouseEnter={(e) => { if (!saving) e.currentTarget.style.backgroundColor = "#222"; }}
                                    onMouseLeave={(e) => { if (!saving) e.currentTarget.style.backgroundColor = "#111"; }}
                                >
                                    {saving ? (
                                        <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> Updating…</>
                                    ) : (
                                        <><SaveOutlinedIcon style={{ fontSize: 16 }} /> Update Password</>
                                    )}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-3">
                            {[
                                ...
                                <div key={i} className="flex items-start gap-3">
                                    ...
                                    >
                                        <span style={{ ...SORA, fontSize: 10, fontWeight: 800, color: "#888" }}>{i + 1}</span>
                                    </div>
                                    <p style={{ ...INTER, fontSize: 13, color: "#666", lineHeight: 1.6 }}>{tip}</p>
                                </div>
                            ))}
                        </div>
                    </SectionCard>
