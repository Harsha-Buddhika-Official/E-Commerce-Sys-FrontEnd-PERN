...
function Card({ children, className = "", style = {} }) {
	return (
		<div className={className} style={{ background: "#ffffff", borderRadius: 20, border: "1px solid #efefef", boxShadow: "0 2px 16px rgba(0,0,0,0.06)", ...style }}>
			{children}
		</div>
	);
}
...
function StepOne({ data, onChange, onNext, onCancel }) {
	...
	const removeImage = (idx) => onChange("images", data.images.filter((_, i) => i !== idx));

	return (
		<div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
			<div style={{ display: "flex", flexDirection: "column", gap: 10, flexShrink: 0 }}>
				{data.images.map((src, idx) => (
					<Card key={idx} className="relative group overflow-hidden" style={{ width: 84, height: 84, borderRadius: 14, padding: 6, display: "flex", alignItems: "center", justifyContent: "center" }}>
						<img src={src} alt="" style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain" }} />
						<button onClick={() => removeImage(idx)} className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-full bg-red-500 text-white transition-all border-none cursor-pointer" style={{ width: 18, height: 18 }}>
							<CloseIcon style={{ fontSize: 11 }} />
						</button>
					</Card>
				))}
				<button onClick={() => fileInputRef.current?.click()} className="flex items-center justify-center cursor-pointer transition-all" style={{ width: 84, height: 84, borderRadius: 14, flexShrink: 0, background: "linear-gradient(135deg,#e8f0fe,#dbeafe)", border: "2px dashed #93c5fd" }}>
			...
			<Card style={{ width: 360, height: 480, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", padding: 16, position: "relative" }}>
				<div style={{ position: "absolute", inset: 0, borderRadius: 20, opacity: 0.4, backgroundImage: "radial-gradient(circle, #e5e7eb 1px, transparent 1px)", backgroundSize: "22px 22px" }} />
				...
				{data.images.length > 0 ? (
					<img src={data.images[0]} alt="preview" style={{ maxHeight: "100%", maxWidth: "100%", objectFit: "contain", position: "relative", zIndex: 1 }} />
				) : (
					<div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, position: "relative", zIndex: 1, cursor: "pointer" }} onClick={() => fileInputRef.current?.click()}>
			...
			<Card style={{ flex: 1, padding: "26px 26px 22px", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 8px 32px rgba(26,115,232,0.10), 0 2px 8px rgba(0,0,0,0.06)", border: "1.5px solid #e0eaff" }}>
				...
				<div>
					...
					<textarea value={data.description} onChange={(e) => onChange("description", e.target.value)} rows={4} placeholder="Product description…" className="w-full px-4 py-3 rounded-xl outline-none resize-none transition-all" style={{ ...INTER, fontSize: 13, fontWeight: 400, background: "#f9f9f9", color: "#374151", border: "1.5px solid #e5e7eb" }} />
				</div>

				<div>
					<Label>Brand</Label>
					<SelectField value={data.brand} onChange={(v) => onChange("brand", v)} placeholder="Select brand" blue options={["MSI","ASUS","Lenovo","Dell","HP","Acer","Apple","Samsung","Intel","AMD","Nvidia","G.Skill","Corsair","Noctua","LG"]} />
				</div>

				<div>
					<Label>Category</Label>
					<SelectField value={data.category} onChange={(v) => onChange("category", v)} placeholder="Select category" blue options={["Laptop","Processor","GPU","Memory","Storage","Monitor","Cooling","Motherboard","PSU","Case","Peripherals"]} />
				</div>
				...
				<div style={{ background: "linear-gradient(135deg,#f8faff 0%,#eff6ff 100%)", borderRadius: 14, padding: "14px 14px 12px", border: "1px solid #dbeafe" }}>
					<p style={{ ...SORA, fontSize: 11, fontWeight: 700, color: "#1a73e8", textTransform: "uppercase", letterSpacing: "0.09em", marginBottom: 12 }}>💰 Pricing</p>
				...
				<div style={{ display: "flex", alignItems: "center", gap: 14 }}>
					...
					<div style={{ display: "flex", alignItems: "center", gap: 12, background: "#f0f6ff", borderRadius: 12, padding: "6px 14px", border: "1.5px solid #c7d9f9" }}>
						<button onClick={() => onChange("quantity", Math.max(0, data.quantity - 1))} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", padding: 0 }}>
							<RemoveIcon style={{ fontSize: 18, color: "#1a73e8" }} />
						</button>
						<span style={{ ...SORA, fontSize: 17, fontWeight: 800, color: "#1a73e8", minWidth: 28, textAlign: "center" }}>{data.quantity}</span>
						<button onClick={() => onChange("quantity", data.quantity + 1)} style={{ background: "none", border: "none", cursor: "pointer", display: "flex", padding: 0 }}>
				...
				<div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 10, borderTop: "1.5px solid #f3f4f6" }}>
					<button onClick={onCancel} style={{ ...INTER, fontSize: 13.5, fontWeight: 600, padding: "9px 24px", borderRadius: 10, background: "#fff", color: "#6b7280", border: "1.5px solid #e5e7eb", cursor: "pointer" }}>Cancel</button>
					<button onClick={onNext} style={{ ...INTER, fontSize: 13.5, fontWeight: 700, padding: "9px 28px", borderRadius: 10, color: "#fff", border: "none", cursor: "pointer", background: "linear-gradient(135deg,#1a73e8,#1e40af)", boxShadow: "0 4px 14px rgba(26,115,232,0.35)" }}>Next →</button>
				</div>
			</Card>
		</div>
	);
...
function AttrCard({ title, icon, badge, children }) {
	return (
		<Card style={{ flex: 1, padding: "26px 26px 22px", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 8px 32px rgba(26,115,232,0.09), 0 2px 8px rgba(0,0,0,0.05)", border: "1.5px solid #e0eaff" }}>
...
function StepTwo({ data, onChange, onUpload, onCancel }) {
	const sectionStyle = { flex: 1, padding: "26px 26px 22px", display: "flex", flexDirection: "column", gap: 16, boxShadow: "0 8px 32px rgba(26,115,232,0.09), 0 2px 8px rgba(0,0,0,0.05)", border: "1.5px solid #e0eaff" };
	return (
		<div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
			<Card style={sectionStyle}>
				<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
					{LEFT_ATTRS.map(({ key, label, options }) => (
						<div key={key}>
							<Label>{label}</Label>
							<SelectField value={data[key] || ""} onChange={(v) => onChange(key, v)} placeholder={`Select ${label.toLowerCase()}…`} options={options} />
						</div>
					))}
				</div>
			</Card>
				...
				<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
					...
					<div>
						...
						<div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
							<SelectField value={data.portsType || ""} onChange={(v) => onChange("portsType", v)} placeholder="Select port type…" options={["Standard","Thunderbolt","USB-C only","Other"]} />
							<textarea value={data.portsDetail || ""} onChange={(e) => onChange("portsDetail", e.target.value)} rows={3} placeholder="e.g. USB-A, USB-C, HDMI, RJ45…" className="w-full px-4 py-3 rounded-xl outline-none resize-none transition-all" style={{ ...INTER, fontSize: 13, color: "#374151", background: "#f9f9f9", border: "1.5px solid #e5e7eb" }} />
						</div>
					</div>
					<div style={{ display: "flex", gap: 10, justifyContent: "flex-end", paddingTop: 10, borderTop: "1.5px solid #f3f4f6", marginTop: 4 }}>
						<button onClick={onCancel} style={{ ...INTER, fontSize: 13.5, fontWeight: 600, padding: "9px 24px", borderRadius: 10, background: "#fff", color: "#6b7280", border: "1.5px solid #e5e7eb", cursor: "pointer" }}>Cancel</button>
						<button onClick={onUpload} style={{ ...INTER, fontSize: 13.5, fontWeight: 700, padding: "9px 28px", borderRadius: 10, color: "#fff", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: 7, background: "linear-gradient(135deg,#1a73e8,#1e40af)", boxShadow: "0 4px 14px rgba(26,115,232,0.35)" }}><CloudUploadOutlinedIcon style={{ fontSize: 16 }} /> Upload</button>
					</div>
				</div>
			</Card>
		</div>
...
const AddProductPage = ({ onCancel = () => {}, onSuccess = () => {} }) => {
	...
	const [product, setProduct] = useState({ images: [], name: "", description: "", brand: "", category: "", basePrice: "", sellingPrice: "", discountedPrice: "", quantity: 10 });
	...
	const handleUpload = () => { onSuccess({ ...product, attributes: attrs }); };

	return (
		<div style={{ height: "100%", overflowY: "auto", background: "#f4f6fb", padding: "28px 28px 40px" }}>
			<div style={{ display: "flex", alignItems: "center", marginBottom: 24, gap: 16 }}>
				<button onClick={() => navigate("/admin/products")} className="flex items-center justify-center w-9 h-9 rounded-xl border border-gray-200 bg-white text-gray-500 hover:border-gray-400 hover:text-gray-700 transition-all cursor-pointer" style={{ flexShrink: 0, borderColor: "#e5e7eb" }}>
				...
				<div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4, background: "#fff", borderRadius: 14, padding: "5px 8px", border: "1px solid #e5e7eb", boxShadow: "0 1px 6px rgba(0,0,0,0.05)" }}>
					{[{ n: 1, label: "Add Product" }, { n: 2, label: "Add Attributes" }].map((s, i, arr) => (
						<div key={s.n} style={{ display: "flex", alignItems: "center", gap: 4 }}>
							...
							>
								<span style={{ width: 19, height: 19, borderRadius: "50%", fontSize: 10, fontWeight: 800, background: step === s.n ? "rgba(255,255,255,0.2)" : step > s.n ? "#e5e7eb" : "#f3f4f6", color: step === s.n ? "#fff" : step > s.n ? "#374151" : "#d1d5db", display: "flex", alignItems: "center", justifyContent: "center" }}>{s.n}</span>
								{s.label}
							</button>
							{i < arr.length - 1 && <div style={{ width: 16, height: 1.5, background: step > 1 ? "#1a73e8" : "#e5e7eb" }} />}
						</div>
					))}
				</div>
			</div>

			{step === 1 && <StepOne data={product} onChange={handleProductChange} onNext={handleNext} onCancel={onCancel} />}
			{step === 2 && <StepTwo data={attrs} onChange={handleAttrsChange} onUpload={handleUpload} onCancel={() => setStep(1)} />}
		</div>
	);
};
