...
export default function DirectCheckout() {
  ...
  const unitPrice  = Number(state?.price ?? 0);
  ...
  const increaseQty = () => setQuantity((q) => Math.min(q + 1, state?.stock ?? 99));
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-white rounded-2xl overflow-hidden sticky top-5" style={{ border: "1px solid #ebebeb", boxShadow: "0 2px 12px rgba(0,0,0,0.04)" }}>
              ...
              <div className="px-6 py-5 flex flex-col gap-4">
                ...
                <div className="flex items-start gap-3 py-2" style={{ borderBottom: "1px solid #f5f5f5" }}>
                  <div className="flex items-center justify-center rounded-lg shrink-0" style={{ width: 36, height: 36, backgroundColor: "#f5f5f5", border: "1px solid #ebebeb" }}>
                    {state.image
                      ? <img src={state.image} alt={state.name} className="w-full h-full object-cover rounded-lg" />
                      : <InventoryOutlinedIcon style={{ fontSize: 16, color: "#bbb" }} />
                    }
                  </div>
                  <div className="flex-1 min-w-0">
                    <p style={{ ...INTER, fontSize: 11, fontWeight: 700, color: "#111", lineHeight: 1.4 }} className="line-clamp-2">{state.name}</p>
                    <p style={{ ...INTER, fontSize: 10, color: "#aaa", marginTop: 2 }}>{formatLKR(unitPrice)} per unit</p>
                  </div>
                </div>

                <div className="flex items-center justify-between">
                  <span style={{ ...INTER, fontSize: 12, color: "#888", fontWeight: 600 }}>Quantity</span>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={decreaseQty} disabled={quantity <= 1}
                      className="flex items-center justify-center w-7 h-7 rounded-lg transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                      style={{ backgroundColor: "#f5f5f5", border: "1px solid #ebebeb" }}
                      onMouseEnter={(e) => { if (quantity > 1) e.currentTarget.style.borderColor = "#111"; }}
                      onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#ebebeb"; }}
                    >
                      <RemoveIcon style={{ fontSize: 14, color: "#555" }} />
                    </button>
                    <span style={{ ...SORA, fontSize: 14, fontWeight: 900, color: "#111", minWidth: 20, textAlign: "center" }}>{quantity}</span>
                    <button
                      onClick={increaseQty} disabled={quantity >= (state?.stock ?? 99)}
                      className="flex items-center justify-center w-7 h-7 rounded-lg transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
                      style={{ backgroundColor: "#f5f5f5", border: "1px solid #ebebeb" }}
                      onMouseEnter={(e) => { if (quantity < (state?.stock ?? 99)) e.currentTarget.style.borderColor = "#111"; }}
                      onMouseLeave={(e) => { e.currentTarget.style.borderColor = "#ebebeb"; }}
                    >
                      <AddIcon style={{ fontSize: 14, color: "#555" }} />
                    </button>
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-2">
                  <div className="flex items-center justify-between py-1.5" style={{ borderBottom: "1px solid #f5f5f5" }}>
                    <span style={{ ...INTER, fontSize: 12, color: "#888", fontWeight: 600 }}>Subtotal ({quantity} item{quantity !== 1 ? "s" : ""})</span>
                    <span style={{ ...INTER, fontSize: 12, color: "#111", fontWeight: 700 }}>{formatLKR(subtotal)}</span>
                  </div>
                  <div className="flex items-center justify-between py-1.5" style={{ borderBottom: "1px solid #f5f5f5" }}>
                ...
                {shipping > 0 && (
                  <div className="flex items-center gap-2 px-3 py-2.5 rounded-xl" style={{ backgroundColor: "#fffbeb", border: "1px solid #fde68a" }}>
                    ...
                    <p style={{ ...INTER, fontSize: 11, fontWeight: 600, color: "#92400e" }}>
                      Add {formatLKR(SHIPPING_THRESHOLD - subtotal)} more for free shipping
                    </p>
                  </div>
                )}
