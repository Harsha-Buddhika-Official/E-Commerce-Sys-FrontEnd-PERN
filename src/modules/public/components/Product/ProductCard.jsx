...
const StockBadge = ({ inStock }) => (
  <div className="absolute bottom-2.25 right-2.5 flex items-center gap-1.25 bg-white border border-[#CCCCCC] rounded-[10px] px-3 py-0.5 h-5">
    ...
    <span
      className={`text-[13px] font-semibold leading-4 flex-none ${inStock ? 'text-[#008000]' : 'text-red-500'}`}
...
const ProductSpecs = ({ specs }) => (
  <div className="text-[13px] font-medium leading-4 text-[#252525] space-y-1" style={{ fontFamily: 'Inter, sans-serif' }}>
...
const ProductCard = ({
  ...
  >
    ...
    <div className="flex flex-col flex-1 px-4 pb-4 pt-2 gap-1">
      <h3
        className="font-bold text-[16px] leading-[1.3] uppercase text-[#252525] line-clamp-2 pb-0.5"
        style={{ fontFamily: 'Inter, sans-serif' }}
      >
        {title}
      </h3>
      ...
      <div className="flex flex-row items-center justify-between mt-1">
        <span className="font-semibold text-[16px] leading-4 text-black" style={{ fontFamily: 'Inter, sans-serif' }}>
          {price}
        </span>
        <div className="flex flex-row items-center gap-2">
          {category && (
            ...
            >
              <span className="font-normal text-[13px] text-[#252525] whitespace-nowrap" style={{ fontFamily: 'Inter, sans-serif' }}>
                {category.split(' ')[0]}
              </span>
            </div>
          )}
